...
а значит не гарантируется стабильная работа методов
...
Для создания строк используются кавычки. Одинарные и двойные кавычки равнозначны

"Hello, world"
'1,2,3'

Чтобы создать длинный текст можно воспользоваться тройными кавычками
...
обращение к элементу списка - через квадратные скобки по индексу 
(порядковому номеру элемента в списке), нумерация начинается с 0:
...
Обращение к элементу - такое же как в списке
...
Зачем кортежи?
Удобны чтобы гарантировать неизменность данных
Быстрее, занимают меньше памяти
...
Удобны для операций включения-исключения. Например, нам нужно найти все объявления с одинаковой группой таргета из двух кампаний
Или нам нужно найти все объявления из одного запроса к API не содержащиеся в другом запросе API

dict - словарь, набор пар ключ-значение. Ключом может быть любой не изменяемый тип данных, значением - любой тип данных
Ключом словаря может быть любой не изменяемый тип данных. Например, строка или целое число. Например, 
кортеж может быть ключом словаря, а множество или список - нет
...
Обращения к элементам словаря - по ключу:
my_dict[1]
my_dict["world"]

bool - логический тип
Позволяет управлять циклами и ветвлениями
Удобно использовать приведение к логическому типу:
Любые числа кроме нуля равны истине:
bool(0) == False #True
bool(1) == True #True
Любые строки кроме пустой - равны истине
Любая коллекция (список, кортеж, словарь, множество) равны истине
